...
class WebController extends Controller
{
    ...
    public function product($slug)
    {
        ...
        if ($product != null) {
            ...
            $getCat = json_decode(($product->category_ids),true)[0]{'id'};
            $menu = Category::with(['childes.childes'])->where('id', $getCat)->first();

            return view('web-views.products.details', compact('product', 'countWishlist', 'countOrder', 'relatedProducts', 'deal_of_the_day', 'menu'));
        }
    ...
    public function products(Request $request)
    {
        ...
        if ($request['id'] == 17 || $request['id'] == 18 || $request['id'] == 19) {
         $menu = Category::with(['childes.childes'])->where('id', $request['id'])->first();
        } else {
         if ($request['id'] && $request['data_from'] != 'brand') {
          $ALLcategory = Category::with(['childes.childes'])->where('id', $request['id'])->first();
          if($ALLcategory->position == 1) {
           $menu = Category::with(['childes.childes'])->where('id', $ALLcategory->parent_id)->first();
          } else {
           $subsub = Category::with(['childes.childes'])->where('id', $ALLcategory->parent_id)->first();
           $menu = Category::with(['childes.childes'])->where('id', $subsub->parent_id)->first();
          }
         }
        }

        $category = Category::with(['childes.childes'])->where('id', $request['id'])->first();

        // $category = Category::with(['childes.childes'])->where('id', $request['id'])->first();

        $featured_products = Product::with(['reviews'])->active()->where('featured', 1)->paginate(20);
        foreach( $featured_products as $fp) {
          // $fpclean = ;
          // dd(json_decode($fp->category_ids)[0]->id);
        }



        return view('web-views.products.view', compact('products', 'data', 'category', 'menu', 'featured_products'), $data);
    }
