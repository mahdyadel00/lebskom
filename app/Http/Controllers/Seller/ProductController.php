        ...
        if ($request->ajax()) {
            ...
            if ($request->file('images')) {
                foreach ($request->file('images') as $key => $img) {
                    ...
                    $disk = Storage::disk('gcs')->put('products/' . $product_images[$key], file_get_contents($img));
                }
                $product->images = json_encode($product_images);
            }
            $product->thumbnail = ImageManager::upload('product/thumbnail/', 'png', $request->file('image'));
            $disk = Storage::disk('gcs')->put('products/thumbnail/' . $product->thumbnail, file_get_contents($request->file('image')));
